    ...
    <div class="flex items-center justify-center py-12">
        <div class="w-full max-w-lg px-6">
            <div class="p-6 bg-white rounded-lg shadow-lg">
                <h3 class="mb-4 text-lg font-semibold">{{ __('Edit Device') }}</h3>
                <form method="POST" action="{{ route('devices.update', $device) }}">
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="device_type" class="block w-full mt-1" type="text" name="device_type"
                            :value="old('device_type', $device->device_type)" required />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="mac_address" class="block w-full mt-1" type="text" name="mac_address"
                            :value="old('mac_address', $device->mac_address)" pattern="^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$"
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="ip_address" class="block w-full mt-1" type="text" name="ip_address"
                            :value="old('ip_address', $device->ip_address)" pattern="^(\d{1,3}\.){3}\d{1,3}$"
                    ...
                    <div class="flex gap-4 mt-4">
                        ...
                        <div class="w-1/2">
                            ...
                            <x-text-input id="height" class="block w-full mt-1" type="number" name="height"
                                :value="old('height', $device->height)" />
                    ...
                    <div class="mt-4">
                        ...
                        <select id="rotation" name="rotation"
                            ...
                            <option value="Landscape"
                                {{ old('rotation', $device->rotation) == 'Landscape' ? 'selected' : '' }}>
                                {{ __('Landscape') }}
                            </option>
                            <option value="Portrait"
                                {{ old('rotation', $device->rotation) == 'Portrait' ? 'selected' : '' }}>
                                {{ __('Portrait') }}
                            </option>
                        </select>
                        ...
                        <select id="location_id" name="location_id"
                            ...
                            @foreach ($locations as $location)
                                <option value="{{ $location->id }}"
                                    {{ old('location_id', $device->location?->id) == $location->id ? 'selected' : '' }}>
